...
<div class='body_div' id='overlay_window_block_body'>
			...
			<div id='zero_amount_booking' class='nn_accordion_section_content'>
				<table class='list table'>
					<tr class='tab_bg1'>
						...
						<td>
							<input type='text' id='book_amount' value='{$customerDetails->fGesamtsumme * 100}' size='7' onkeypress='return isNumberKey(event)'>
							<button type='button' class='confirm' id='amount_book' onclick='captureval({$nnOrderno},"{$apiCode['book']}")'> Buchen </button>
						</td>
					</tr>
				</table>
			</div>
	...
	{else}
			...
			<div class='nn_accordion_section'>
				...
				<div id='transaction_confirmation' class='nn_accordion_section_content'>
					<table class='list table'>
						<tr class='tab_bg1'>
							<td>
								<button type='button' id='capture' class='confirm' onclick='captureval({$nnOrderno},"{$apiCode['debit']}")'> Best&auml;tigen </button>
								<button type='button' id='void' class='confirm' onclick='captureval({$nnOrderno},"{$apiCode['cancel']}")'> Stornieren </button>
							</td>
						</tr>
					</table>
				</div>
		...
		{if $orderInfo->nStatuswert == 100 && $callbackInfo->totalAmount >= $orderInfo->nBetrag}
			<div class='nn_accordion_section'>
				...
				<div id='transaction_refund' class='nn_accordion_section_content'>
					<table class='list table'>
					{if $orderInfo->cZahlungsmethode|in_array:(array('novalnet_invoice','novalnet_ideal','novalnet_banktransfer','novalnet_prepayment'))}
						<tr class='tab_bg1'>
					...
					{/if}			
						...
						<tr class='tab_bg1'>
							<td></td>
							<td colspan=3>
								<button type='button' class='confirm' id='refund_update' onclick='refundval({$nnOrderno},"{$apiCode['refund']}")'> Best&auml;tigen </button>
							</td>
						</tr>
					</table>
				</div>
				...
				<div id='transaction_update' class='nn_accordion_section_content'>
					<table class='list table'>
						...
						<tr class='tab_bg1'>
							<td></td>
							<td colspan=3>
								<button name='amount_update' type='button' class='confirm' id='amount_update' onclick='amountupdate({$nnOrderno},"{$apiCode['amountChange']}")'> &Auml;ndern </button>
							</td>
						</tr>
					</table>
				</div>
			...
			<div class='nn_accordion_section'>
				...
				<div id='transaction_subs_cancellation' class='nn_accordion_section_content'>
					<table class='list table'>
						...
						<tr class='tab_bg1'>
							<td></td>
							<td><button type='button' class='confirm' id='subs_cancel' onclick='subscriptionCancel({$nnOrderno},"{$apiCode['subscription']}")'> Best&auml;tigen </button></td>
						</tr>
					</table>
				</div>
			</div>
