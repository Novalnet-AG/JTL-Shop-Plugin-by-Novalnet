...
jQuery(document).ready(function() {
	...
	jQuery('#'+paymentFormId).submit(function() {
		...
		if ( jQuery('#nn_new_card_details').css('display') == 'block' ) {
			...
			if ( jQuery('#nn_sepa_saved_details_confirm') && !( jQuery('#nn_sepa_saved_details_confirm').is(':checked') ) ) {
				alert( jQuery('#nn_lang_mandate_confirm').val() );
				return false;
			}
		}
...
{	
	...
	var ibanBicRequestParams = { 'account_holder' : account_holder , 'bank_account' : account_no , 'bank_code' : bank_code, 'vendor_id' : jQuery('#novalnet_vendor').val(), 'vendor_authcode' : jQuery('#novalnet_authcode').val(), 'bank_country' : bank_country, 'unique_id' : jQuery('#nn_sepaunique_id').val(), 'get_iban_bic' : 1 };
	...
	if ( sepaCallResponseParams.hash_result == 'success' ) {
		...
		switch ( type ) {
			...
			case 'hash':
				...
				jQuery('#nn_payment_hash').attr( 'disabled', false );
...
function sepaRefillFormcall()
{
	...
	nn_vendor = jQuery('#novalnet_vendor').length ? jQuery('#novalnet_vendor').val() : '';
	nn_authcode = jQuery('#novalnet_authcode').length ? jQuery('#novalnet_authcode').val() : '';
	nn_sepaunique_id = jQuery('#nn_sepaunique_id').length ? jQuery('#nn_sepaunique_id').val() : '';
		
	if ( nn_vendor == '' || nn_authcode == '' || nn_sepaunique_id == '' )
		return false;

	jQuery('#nn_loader').css('display','block');
	
	var refillRequestParams = { 'vendor_id' : nn_vendor , 'vendor_authcode' : nn_authcode , 'unique_id' : nn_sepaunique_id, 'sepa_data_approved' : 1, 'mandate_data_req' : 1, 'sepa_hash' : refillpanhash };
