...
class NovalnetCreditcard extends PaymentMethod
{
    ...
    public function handleAdditional($aPost_arr)
    {
        ...
        if ($this->novalnetGateway->helper->getConfigurationParams('extensive_option', $this->paymentName) == '1'
            && !empty($referenceTid)
            && (!($this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode')
            && $this->novalnetGateway->helper->getConfigurationParams('cc3d_fraud_check')))) {                          // Condition to display saved card details
    ...
    public function preparePaymentProcess($order)
    {
        ...
        if (empty($_SESSION[$this->paymentName]['one_click_shopping'])) { // Condition to check reference transaction
            if (($extensiveOption == '1' && !($this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode')
    ...
    {
        return base64_encode($this->novalnetGateway->helper->getConfigurationParams('novalnet_public_key') . '&' .
            nnGetIpAddress('REMOTE_ADDR') . '&' .nnGetIpAddress('SERVER_ADDR'));
    }
    ...
    public function getDynamicCreditCardFormFields()
    {
        ...
        $textFields = array(
                        'credit_card_name',
                        'credit_card_name_input',
                        'credit_card_number',
                        'credit_card_number_input',
                        'credit_card_date',
                        'credit_card_date_input',
                        'credit_card_cvc',
                        'credit_card_cvc_input',
                        'credit_card_cvc_hint',
                        'credit_card_error'
                    );

        foreach ($textFields as $value) {
            $ccformFields[$value] = utf8_encode(
                $this->novalnetGateway->helper->oPlugin->oPluginSprachvariableAssoc_arr['__NN_' . $value]
            );
        }
