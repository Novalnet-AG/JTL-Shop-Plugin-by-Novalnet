...
if (!empty($request['bestellung'])) { // Condition to check whether the page is my-account page
    ...
    if (is_object($subscriptionValue)
        && $subscriptionValue->nSubsId != '0'
        && $subscriptionValue->nStatuswert <=100
        && $subscriptionValue->cTerminationReason == null) { // Condition to check whether order is a subscription order
        $placeholder = array(
                            '__NN_subscription_title',
                            '__NN_subscription_reasons',
                            '__NN_subscription_reason_error',
                            '__NN_select_type',
                            '__NN_subscription_reason_error',
                            '__NN_confirm_btn'
                    );

        $subsReasonPlaceholder = array(
                                    '__NN_subscription_offer_expensive',
                                    '__NN_subscription_fraud',
                                    '__NN_subscription_partner_intervened',
                                    '__NN_subscription_financial_problems',
                                    '__NN_subscription_content_not_meeting_expectations',
                                    '__NN_subscription_content_not_sufficient',
                                    '__NN_subscription_interest_on_test_access',
                                    '__NN_subscription_page_slow',
                                    '__NN_subscription_satisfied',
                                    '__NN_subscription_access_problems',
