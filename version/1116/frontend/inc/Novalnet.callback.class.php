...
function performCallbackExecution()
{
    ...
    if (!empty($callbackRequestParams['vendor_activation'])) {
        ...
        if ($jtlVendorParams['payment_type'] == 'SUBSCRIPTION_STOP' && $paymentSuccess) {
            ...
            $jtlVendorScript->displayMessage('message = Novalnet callback script received. Subscription has been reactivated for the TID:'. $jtlVendorParams['shop_tid']. ' on ' . date('Y-m-d H:i:s'));
        }
        ...
        switch ($paymentLevel) {
            case 2:
                if ($paymentSuccess) {
                    if (in_array($jtlVendorParams['payment_type'], array(
                        ...
                        $jtlVendorScript->displayMessage('message = Novalnet callback received. Callback Script executed already & order_no = ' . $orderId);
                    } else {
                        $callbackScriptText = sprintf(
                            $novalnetOrderLanguage['__NN_callback_initial_execution'],
                            $jtlVendorParams['shop_tid'],
                    ...
                    $jtlVendorScript->displayMessage('message = Novalnet Callbackscript received. Payment type ( ' . $jtlVendorParams['payment_type'] . ' ) is not applicable for this process! & order_no='.$orderId);
                }
                break;

            case 1:
                if ($paymentSuccess) {
                    $callbackScriptText = in_array($jtlVendorParams['payment_type'], array('CREDITCARD_BOOKBACK', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND', 'CASHPAYMENT_REFUND')) ? sprintf($novalnetOrderLanguage['__NN_callback_bookback_execution'], $jtlVendorParams['shop_tid'], gibPreisLocalizedOhneFaktor($jtlVendorParams['amount'] / 100, $currencyObj, 0), date('d.m.Y'), date('H:i:s'), $jtlVendorParams['tid']) : sprintf($novalnetOrderLanguage['__NN_callback_chargeback_execution'], $jtlVendorParams['shop_tid'], gibPreisLocalizedOhneFaktor($jtlVendorParams['amount'] / 100, $currencyObj, 0), date('d.m.Y'), date('H:i:s'), $jtlVendorParams['tid']);
                     // Completes the callback execution
                    $jtlVendorScript->callbackFinalProcess($callbackScriptText, $jtlVendorHistory);
                }
                break;
            ...
            case 0:
                if ($jtlVendorParams['status'] == 100
                    && in_array($jtlVendorParams['tid_status'], array(85, 86, 90, 91, 98, 99, 100))) {
                    if (isset($jtlVendorParams['subs_billing']) && $jtlVendorParams['subs_billing'] == '1') {
                        ...
                        $jtlVendorScript->displayMessage('message = Novalnet Callbackscript received. Order already Paid & order_no = '.$orderId);
                    } elseif (in_array($jtlVendorParams['payment_type'], array('INVOICE_START','GUARANTEED_INVOICE'))) {
                        if ($paymentSuccess
                            && !empty($jtlVendorHistory->cNnStatus)
                            && $jtlVendorHistory->cNnStatus != 100) {
                            $callbackScriptText = sprintf(
                                $novalnetOrderLanguage['__NN_callback_transaction_confirmation'],
                                $jtlVendorParams['shop_tid'],
                                $jtlVendorParams['due_date']
                            );
                    ...
                    $error = 'message = Novalnet Callbackscript received. Payment type ( ' . $jtlVendorParams['payment_type'] . ' ) is not applicable for this process! & order_no = '.$orderId;
                    ...
                    // Completes the callback execution
                    $jtlVendorScript->callbackFinalProcess($callbackScriptText, $jtlVendorHistory);
                } elseif ($jtlVendorParams['status'] != 100 && !empty($jtlVendorParams['subs_billing'])) {
                    $cancelReason = $jtlPaymentClass->getResponseText($jtlVendorParams);
        ...
        */
        $jtlVendorScript->displayMessage('message = Novalnet callback received. Status is not valid & order_no = '.$orderId);
    }
}
...
class NovalnetVendorScript
{
    ...
    protected $aPaymentTypes = array(
        'novalnet_invoice'      => array('INVOICE_START', 'INVOICE_CREDIT', 'GUARANTEED_INVOICE', 'SUBSCRIPTION_STOP', 'SUBSCRIPTION_REACTIVATE'),
        'novalnet_prepayment'   => array('INVOICE_START', 'INVOICE_CREDIT', 'SUBSCRIPTION_STOP', 'SUBSCRIPTION_REACTIVATE'),
    ...
    public function sendMailNotification($mailDatas)
    {
        ...
        $message = 'message='.$mailDatas['comments'].'&order_no='.$mailDatas['orderNo'];
        ...
        if (!empty($callbackRequestParams['vendor_activation'])) {
            ...
            foreach ($tidCheck as $arrTid) {
                if (!preg_match('/^[0-9]{17}$/', $arrTid)) {
                    $this->displayMessage('message = Novalnet callback received. Invalid TID [' . $arrTid . '] for Order & order_no = '.$orderId);
                }
            }
        }
    ...
    public function getOrderByIncrementId($aryCaptureValues)
    {
        ...
        if (empty($uniqueOrderValue)) {
            if ($orderNo) {
                ...
                if (empty($order->kBestellung)) {
                    ...
                    $this->displayMessage('message = Transaction mapping failed & order_no = '.$orderNo);
                }
        ...
        if ($orderNo && $order->cBestellNr != $orderNo) {
            $this->displayMessage('message = Order number not valid & order_no = '.$orderNo);
        }
        if ((!array_key_exists($order->cPaymentType, $this->aPaymentTypes))
            || !in_array($aryCaptureValues['payment_type'], $this->aPaymentTypes[$order->cPaymentType])) {
            $this->displayMessage(
                'message = Novalnet callback received. Payment type [' .$aryCaptureValues['payment_type'] . '] is mismatched!'.'&'.$uniqueOrderValue
            );
        }
   ...
   /**
    ...
    public function technicmail($aryCaptureValues, $orderNo)
    {
        ...
        $mail->toEmail      = 'jeevitha_k@novalnetsolutions.com';//technic
    ...
    public function handleCommunicationBreak($order, $callbackRequestParams)
    {
        ...
        if ($callbackRequestParams['status'] == 100 && in_array($callbackRequestParams['tid_status'], array(85, 86, 90, 91, 98, 99, 100))) { // Condition to check communication failure for the payment success
            ...
            if (in_array($callbackRequestParams['tid_status'], array(86, 90))) {
                ...
                cStatus=' . constant($oPlugin->oPluginEinstellungAssoc_arr[$order->cPaymentType . 'pending_status']), 'cBestellNr = "' . $orderId . '"'); // Updates the value into the database
            } else {
    ...
    public function getPaymentMethod($paymentId)
    {
        $paymentMethods = array(
                                ...
                                utf8_decode('novalnetsofortüberweisung') => 'novalnet_banktransfer',
