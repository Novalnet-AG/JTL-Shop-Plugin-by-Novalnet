...
require_once(PFAD_ROOT. PFAD_CLASSES . 'class.JTL-Shop.Plugin.php');
    ...
    */
    ...
    {
    global $processTestMode, $jtlPaymentClass, $DB, $shopQuery, $novalnetOrderLanguage, $selectQuery;
    ...
    if($jtlVendorParams['payment_type'] == 'TRANSACTION_CANCELLATION') {
    ...
    switch ($paymentLevel) {
        ...
        case '0':
            if ($jtlVendorParams['status'] == 100 && in_array($jtlVendorParams['tid_status'], array(85, 86, 90, 91, 98, 99, 100))) {
                if (in_array($jtlVendorParams['payment_type'], array('PAYPAL', 'PRZELEWY24')) && $jtlVendorHistory->cNnStatus != 100) {
                    $query = $DB->$shopQuery('SELECT SUM(nCallbackAmount) AS kCallbackAmount FROM xplugin_novalnetag_tcallback WHERE cBestellnummer = "' . $orderId . '"', 1);
                ...
                } elseif (in_array($jtlVendorParams['payment_type'], array('INVOICE_START','GUARANTEED_INVOICE', 'DIRECT_DEBIT_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'CREDITCARD'))) {

                    if(in_array($jtlVendorParams['tid_status'], array(91,99,100)) && in_array($jtlVendorHistory->cNnStatus, array('75','91','99','98'))){
                        $ordStatus = constant($oPlugin->oPluginEinstellungAssoc_arr[$jtlVendorHistory->cPaymentType.'_set_order_status']);
                        if($jtlVendorParams['tid_status'] == 100 && $jtlVendorHistory->cNnStatus == 91){
                            ...
                            $ordStatus = $jtlVendorParams['tid_status'] == 100 ? constant($oPlugin->oPluginEinstellungAssoc_arr[$jtlVendorHistory->cPaymentType.'_set_order_status']) : constant($oPlugin->oPluginEinstellungAssoc_arr['confirm_order_status']);
                        ...
                        } elseif ($jtlVendorParams['payment_type'] == 'CREDITCARD') {
                            $callbackScriptText = sprintf(
                                $novalnetOrderLanguage['__NN_order_confirmation_text'],
                                date('d.m.Y'),
                                date('H:i:s')
    ...
    */
    ...
    'novalnet_invoice'      => array('INVOICE_START', 'INVOICE_CREDIT', 'GUARANTEED_INVOICE',  'REFUND_BY_BANK_TRANSFER_EU', 'GUARANTEED_INVOICE_BOOKBACK', 'CREDIT_ENTRY_DE', 'DEBT_COLLECTION_DE'),
    ...
    'novalnet_ideal'        => array('IDEAL', 'REFUND_BY_BANK_TRANSFER_EU', 'ONLINE_TRANSFER_CREDIT', 'CREDIT_ENTRY_DE', 'DEBT_COLLECTION_DE', 'REVERSAL'),
    'novalnet_sepa'         => array(
                                    'DIRECT_DEBIT_SEPA',
                                    'GUARANTEED_DIRECT_DEBIT_SEPA',
                                    'RETURN_DEBIT_SEPA',
    ...
    */
    ...
    $this->affParamsRequired = array(
     ...
     if (isset($aryCapture['payment_type'])
