...
class NovalnetSepa extends PaymentMethod
{
    ...
    public function handleAdditional($aPost_arr)
    {
        ...
        $smarty->assign(array(
            ...
            'uniqValue'           => nnGetRandomString(), // Generates unique ID for the payment
            ...
            'countryList'         => gibBelieferbareLaender($_SESSION['Kunde']->kKundengruppe),
        ...
        if ($this->novalnetGateway->helper->getConfigurationParams('extensive_option', $this->paymentName) == '1' && empty($_SESSION[$this->paymentName]['tid']) && !empty($referenceTid)) { // Condition to check reference transaction
    ...
    public function preparePaymentProcess($order)
    {
        ...
        if (!empty($_SESSION['nn_' . $this->paymentName . '_tid'])) {
            ...
            if (!empty($_SESSION[$this->paymentName]['one_click_shopping'])) { // Condition to reference transaction
                ...
                if ($extensiveOption == '1' || ($extensiveOption == '2' &&
    ...
    public function updateShopDatabase($order)
    {
        ...
        NovalnetGateway::performDbExecution('tbestellung', 'cStatus='.constant((!empty($_SESSION['nn_' . $this->paymentName . '_guarantee']) && $_SESSION[$this->paymentName]['tid_status'] == 75) ? $this->novalnetGateway->helper->getConfigurationParams('guarantee_pending_status', $this->paymentName) : $this->novalnetGateway->helper->getConfigurationParams('set_order_status', $this->paymentName)), 'cBestellNr = "' . $order->cBestellNr . '"');// Updates the value into the database                                                                                                             
    }
    ...
    public function getSepaRefillHash()
    {
        ...
        if ($this->novalnetGateway->helper->getConfigurationParams('sepa_refill')) {                                    // Condition to check if the SEPA payment refill is enabled
            $storedHash = $DB->$shopQuery('SELECT cSepaHash FROM xplugin_novalnetag_tnovalnet_status WHERE cMail = "'
                . $_SESSION['Kunde']->cMail . '" ORDER BY kSno DESC LIMIT 1', 1);
        }

        return (($this->novalnetGateway->helper->getConfigurationParams('sepa_autorefill') &&
                !empty($_SESSION[$this->paymentName]['nn_payment_hash'])) ?
                $_SESSION[$this->paymentName]['nn_payment_hash'] :
                (!empty($storedHash) ? $storedHash->cSepaHash : '')
            );
    }
    ...
    {
        return (nnIsDigits($dueDate) && $dueDate > 6) ?
