...
jQuery(document).ready(function () {
    ...
    jQuery.each(['select[name=novalnet_invoice_guarantee]', 'select[name=novalnet_sepa_guarantee]'], function (index, element) {
        jQuery(element).parent().parent().prepend(decodeTextMessage('<div class="nn_additional_div"><b><h4>Grundanforderungen für die Zahlungsgarantie</h4><br>Zugelassene Staaten: AT, DE, CH<br> Zugelassene Währung: EUR<br>Mindestbetrag der Bestellung >= 9,99 EUR<br>Mindestalter des Endkunden >= 18 Jahre<br>Rechnungsadresse und Lieferadresse müssen übereinstimmen<br>Geschenkgutscheine / Coupons sind nicht erlaubt</b></div>'));
    });
    ...
    var validationElements = ['sepa_due_date', 'novalnet_cc_manual_check_limit', 'novalnet_sepa_manual_check_limit', 'novalnet_invoice_manual_check_limit', 'novalnet_paypal_manual_check_limit', 'novalnet_invoice_guarantee_min_amount', 'novalnet_sepa_guarantee_min_amount', 'tariff_period', 'tariff_period2', 'tariff_period2_amount', 'novalnet_public_key'];
        ...
        if (jQuery.trim(jQuery('input[name=sepa_due_date]').val()) != '' && (isNaN(jQuery('input[name=sepa_due_date]').val()) || jQuery('input[name=sepa_due_date]').val() < 7)) {
            ...
            jQuery.each(['invoice', 'sepa'],function (index, value) {
                if (jQuery('select[name=novalnet_'+value+'_guarantee]').val() == 1) {
                    ...
                    if (minimum_guarantee_amount != '' && (minimum_guarantee_amount < 999)) {
                        event.preventDefault();
                        event.stopPropagation();
            ...
            jQuery.each(['cc', 'sepa', 'invoice', 'paypal'],function (index, value) {
                if (jQuery.trim(jQuery('input[name=novalnet_'+value+'_manual_check_limit]').val()) != '' && isNaN(jQuery.trim(jQuery('input[name=novalnet_'+value+'_manual_check_limit]').val()))) {
                    event.preventDefault();
                    event.stopPropagation();
                    alert(decodeTextMessage('Füllen Sie bitte alle Pflichtfelder aus'));
                    handleErrorElement(jQuery('input[name=novalnet_'+value+'_manual_check_limit]'));
                }
            });
        }
    }
