        ...
        switch ($request['apiStatus'])
        {
                ...
                if ($request['apiStatus'] == 'capture') {
                    ...
                    if (!in_array($paymentMethod, array('novalnet_invoice', 'novalnet_prepayment')) || ($paymentMethod == 'novalnet_paypal' && $transactionResponse['tid_status'] == 100)) { // Pending payments will be added to incoming transactions
                        ...
                        if ($transactionResponse['due_date']) {
                            $transactionResponseMessage = sprintf($novalnetOrderLanguage['__NN_transaction_capture_duedate_text'], $extensionServerRequest['tid'], $transactionResponse['due_date']);
            ...
            case 'amountUpdate': // Amount update process for transaction
                ...
                if ($paymentMethod == 'novalnet_sepa') {
                    ...
                    $transactionResponseMessage = sprintf($novalnetOrderLanguage['__NN_amount_update_message'], gibPreisLocalizedOhneFaktor($extensionServerRequest['amount'] / 100, 0, 0), date('d.m.Y'), date('H:i:s'));

                } else {

                    $updatedDate = new DateTime($extensionServerRequest['due_date']);

                    $transactionResponseMessage = sprintf($novalnetOrderLanguage['__NN_amount_duedate_update_message'], gibPreisLocalizedOhneFaktor($extensionServerRequest['amount'] / 100, 0, 0), $updatedDate->format('d.m.Y'));
                }
                ...
                if ($paymentMethod ==  'novalnet_cashpayment') {
                    ...
                    $updatedStoreComments = str_replace($convertedDate, $updatedDate->format('d.m.Y'), $cashpaymentInfo['cashpaymentStoreDetails']);
            ...
            case 'zeroBooking': // Zero-amount process for transaction
                ...
                $transactionResponseMessage = sprintf($novalnetOrderLanguage['__NN_zero_booking_message'], (gibPreisLocalizedOhneFaktor($transactionResponse['amount'], 0, 0) . ' ' . $currency), $extensionServerRequest['tid']);
            ...
            default: // Refund process for transaction
                ...
                $transactionResponseMessage = sprintf($novalnetOrderLanguage['__NN_refund_message'], $extensionServerRequest['tid'], gibPreisLocalizedOhneFaktor($request['refundAmount'] / 100, 0, 0));
