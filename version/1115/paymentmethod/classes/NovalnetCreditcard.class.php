...
class NovalnetCreditcard extends PaymentMethod
{
    ...
    public function handleAdditional($aPost_arr)
    {
        ...
        if ($this->novalnetGateway->helper->getConfigurationParams('extensive_option', $this->paymentName) == '1' && !empty($referenceTid) && (!($this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode') && $this->novalnetGateway->helper->getConfigurationParams('cc3d_fraud_check')))) { // Condition to display saved card details
    ...
    public function preparePaymentProcess($order)
    {
        ...
        if (empty($_SESSION[$this->paymentName]['one_click_shopping'])) { // Condition to check reference transaction
            ...
            if (($extensiveOption == '1' && !($this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode') && $this->novalnetGateway->helper->getConfigurationParams('cc3d_fraud_check'))) || $extensiveOption == '2') {
    ...
    public function finalizeOrder($order, $hash, $args)
    {
        ...
        return $this->novalnetGateway->verifyNotification($order, $this->paymentName, $this->paymentKey, $this->is3dSecureEnabled() ? $args : ''); // Finalises the order based on response
    }
    ...
    public function handleNotification($order, $paymentHash, $args)
    {
        ...
        if (is_object($incomingPayment) && intval($incomingPayment->kZahlungseingang) > 0) {
            ...
            $this->novalnetGateway->handlePaymentCompletion($order, $this->generateHash($order), $this->paymentKey, $this->paymentName, $this->is3dSecureEnabled() ? $args : ''); // Completes the order
        }
    }
