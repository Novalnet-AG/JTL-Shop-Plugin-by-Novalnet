...
class NovalnetGateway
{
    ...
    public function generatePaymentParams($order, $payment)
    {
        ...
        $telNumber = !empty($_SESSION[$payment]['nn_tel_number']) ? $_SESSION[$payment]['nn_tel_number'] : $_SESSION['Kunde']->cTel;

        if ($telNumber != '') {
            $paymentRequestParameters['tel'] = $telNumber;
        }

        $mobNumber = !empty($_SESSION[$payment]['nn_mob_number']) ? $_SESSION[$payment]['nn_mob_number'] : $_SESSION['Kunde']->cMobil;
        ...
        $txnReference2 = $this->helper->getConfigurationParams('reference2', $payment);
    ...
    public function redirectOnError($order, $response, $paymentName)
    {
        ...
        if ($_SESSION['Zahlungsart']->nWaehrendBestellung == 0 || !empty($_SESSION['nn_during_order'])) {
            ...
            $this->helper->novalnetSessionCleanUp($paymentName); // Unsets the entire novalnet session on order completion
        ...
        if (!empty($response['status'])) { // Evaluate if the status is returned during notify call
            ...
            if (in_array($response['status'], array('90', '100'))) { // On successful payment server response
                ...
                if (!empty($_SESSION['nn_during_order'])) {
                    ...
                    $transactionComments = $this->formTransactionComments($response, $order, $paymentName); // Form transaction comments for the current order
                ...
                $this->insertOrderIntoDB($response, $order->kBestellung, $paymentKey, $paymentName); // Logs the order details in Novalnet tables for success
                ...
                if ($this->helper->oPlugin->oPluginEmailvorlageAssoc_arr['novalnetnotification']->cAktiv == 'Y' && $this->helper->getConfigurationParams('testmode', $paymentName) == '0' && $response['test_mode'] == '1') {
                    ...
                    sendeMail('kPlugin_' . $this->helper->oPlugin->kPlugin . '_novalnetnotification', $oMail); // Triggers email notification to merchant when the order placed as test order
                }
                ...
                $this->insertOrderIntoDBForFailure($response, $order, $paymentName, $paymentKey); // Logs the order details in Novalnet tables for failure
    ...
    public function verifyNotification($order, $payment, $paymentKey, $response = array())
    {
        ...
        if (in_array($response['status'], array('90', '100'))) { // On successful payment server response
            ...
            if (!empty($response['hash2'])) {
                $this->hashCheckForRedirects($response, $order, $payment); // Compares the hash generated for redirection payments
            }
            ...
            if (!empty($_SESSION['nn_during_order'])) {
                $this->insertOrderIntoDBForFailure($response, $order, $payment, $paymentKey); // Logs the order details in Novalnet tables for failure
            }
    ...
    public function formTransactionComments($response, $order, $paymentName, $onError = false)
    {
        ...
        if (!empty($_SESSION['cPost_arr']['kommentar'])) {
            $userComments = (empty($_SESSION['nn_during_order']) ? $_SESSION['cPost_arr']['kommentar'] : '') . PHP_EOL . PHP_EOL;
        }
        $transactionComments = $userComments . $order->cZahlungsartName . PHP_EOL; // Concatenate the Novalnet transaction comments with the user comments if given
        ...
        if (!empty($response['tid'])) {
            $transactionComments .= $this->helper->oPlugin->oPluginSprachvariableAssoc_arr['__NN_tid_label'] . $response['tid'] . PHP_EOL;
        }

        $this->getPaymentTestmode($response); // Returns test mode value based on the payment type

        if (!empty($response['test_mode']) || $this->helper->getConfigurationParams('testmode', $paymentName) != '0') { // Condition to retrieve the testmode for the payment
    ...
    public function formInvoicePrepaymentComments($response, $currency, $updateAmount = false)
    {
        ...
        $transComments .= $novalnetOrderLanguage['__NN_invoice_payments_amount'] . number_format($response['amount'], 2, ',', '') . ' ' . $currency . PHP_EOL;

        $paymentName     = ($response['invoice_type'] == 'PREPAYMENT') ? 'novalnet_prepayment' : 'novalnet_invoice';
        $referenceParams = $updateAmount ? unserialize($response['referenceValues']) : $this->helper->getInvoicePaymentsReferences($paymentName);
        $refCount        = array_count_values($referenceParams);
        $referenceSuffix = array('BNR-' . $this->product . '-' . $orderNo, 'TID ' . $response['tid'], $novalnetOrderLanguage['__NN_order_number_text'] . $orderNo );
        $i = 1;
        ...
        foreach ($referenceParams as $key => $val) {
            if (!empty($val)) {
                $suffix = (nnLoadLanguageIso($response['kSprache']) == 'ger' && $refCount['1'] > 1) ? $i . '. ' : ($refCount['1'] > 1 ? ' ' .$i : '');
    ...
    public function prepareCashpaymentComments($response)
    {
        ...
        $duedate = new DateTime($response['cashpayment_due_date']);
        $transComments  = ($response['cashpayment_due_date'] != '') ? $novalnetOrderLanguage['__NN_cashpayment_expiry_date'] . $duedate->format('d.m.Y') . PHP_EOL : '';
        ...
        for ($i = 1; $i <= $storeCount; $i++) {
            ...
            $transComments .= PHP_EOL . (nnLoadLanguageIso($response['kSprache']) == 'ger' ? $countryName->cDeutsch : $countryName->cEnglisch). PHP_EOL;
        }
    ...
    public function getPinStatus($payment)
    {
        ...
        $fraudModuleParams  = '<?xml version="1.0" encoding="UTF-8"?><nnxml><info_request><vendor_id>' . $this->vendor . '</vendor_id>';
    ...
    public function insertOrderIntoDB($response, $orderValue, $paymentKey, $paymentName)
    {
        ...
        $insertOrder->cKonfigurations      = serialize(array('vendor' => $this->vendor, 'auth_code' => $this->auth_code, 'product' => $this->product, 'tariff' => $this->tariff, 'key' => $paymentKey));
        ...
        $insertOrder->nBetrag              = !is_numeric($response['amount']) ? $this->helper->generateDecode($response['amount']) : ((!empty($response['amount']) ? $response['amount'] : $_SESSION['nn_amount']) * 100);
        ...
        if (!in_array($paymentKey, array('27', '41', '78', '59')) && $response['status'] == '100') { // If the payment server response is successful
     ...
     * @return array
     */
    function setupAdditionalInfoData($response, $paymentKey, $referenceOrder, $paymentName)
    {
        if ($paymentKey != '59' && ($this->helper->getConfigurationParams('extensive_option', $paymentName) != '1' || $referenceOrder || empty($_SESSION['Kunde']->kKunde))) {
            return '';
        }
        ...
        switch($paymentKey)
        {
            ...
            case '59':
                $cashpaymentComments  = $this->prepareCashpaymentComments($response);
                return serialize(array(
                    'cashpaymentSlipduedate'  => $response['cashpayment_due_date'],
    ...
    {
        return (utf8_decode(!empty($response['status_desc']) ? $response['status_desc'] : (!empty($response['status_text']) ? $response['status_text'] : (!empty($response['status_message']) ? $response['status_message'] : $this->helper->oPlugin->oPluginSprachvariableAssoc_arr['__NN_transaction_error']))));
    }
    ...
    public function basicValidationOnhandleAdditional($paymentType)
    {
        ...
        if (!empty($_SESSION['nn_' . $paymentType . '_guarantee']) && !empty($_SESSION['nn_' . $paymentType . '_guarantee_error'])) {
            $_SESSION['nn_error'] = $this->helper->oPlugin->oPluginSprachvariableAssoc_arr['__NN_guarantee_basic_error'];
