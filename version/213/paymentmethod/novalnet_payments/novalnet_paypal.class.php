...
class novalnet_paypal extends novalnetgateway {
  ...
  function __construct() {
    ...
    if (isset($_REQUEST['status']) && $_REQUEST['status'] && $_REQUEST['paymentname'] == 'novalnet_paypal' && isset($_SESSION['novalnet']['nWaehrendBestellung']) && $_SESSION['novalnet']['nWaehrendBestellung'] == 'Nein') {
      $this->orderComplete($_REQUEST);
    }
    if (isset($_REQUEST['status']) && ($_REQUEST['status'] != 100 || $_REQUEST['status'] != 90) && !empty($_REQUEST['status_text']) && $_REQUEST['paymentname']=='novalnet_paypal') {
