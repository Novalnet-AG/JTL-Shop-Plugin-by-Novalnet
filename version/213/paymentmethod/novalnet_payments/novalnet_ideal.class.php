...
#                                                       #
#  Script : novalnet_ideal.class.php                    #
#                                                       #
#########################################################
...
ob_start();
class novalnet_ideal extends novalnetgateway {
  ...
  public $testmode;
  public $key_password;
  public $payment_name = 'novalnet_ideal';

  function __construct() {
    global $DB, $cEditZahlungHinweis, $smarty, $hinweis, $oPlugin;

    $this->name = NOVALNET_IDEAL_WAWI_NAME;
    ...
    if (isset($_SESSION['novalnet']['nWaehrendBestellung']) && $_SESSION['novalnet']['nWaehrendBestellung'] == 'Nein') {
      if (isset($_REQUEST['status']) && $_REQUEST['status'] && $_REQUEST['paymentname'] == 'novalnet_ideal') {
        $this->orderComplete($_REQUEST);
      }
    }
    if (isset($_REQUEST['status']) && $_REQUEST['status'] != 100 && !empty( $_REQUEST['status_text'] ) && $_REQUEST['paymentname'] == 'novalnet_ideal') {
  ...
  function preparePaymentProcess($order) {
    ...
    $order_no = $Zahlung_vor_Bestell= '';
    $params = array();
    $this->novalnet_session_unset();
    $order_update = $this->returnOrderType();
    if ($order_update == true) {
      $Zahlung_vor_Bestell = $this->checkOrderOnUpdate($order, 'novalnetideal');
    }
