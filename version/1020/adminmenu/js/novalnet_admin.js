...
function captureval( api_orderno, capture_code ) {
    var params = {'orderNo' : api_orderno ,'pluginInc': jQuery('#nn_plugin_include').val(),'apiStatus' : capture_code };
    ajax_call(params);
}
...
function amountupdate( api_orderno, amount_update ) {
    ...
    var params = {'orderNo' : api_orderno ,'pluginInc': jQuery('#nn_plugin_include').val(),'apiStatus' : amount_update ,'amount' : amount_value ,'dueDateChange' : duedate_value};
    ajax_call(params);
}
...
function refundval( api_orderno, refund_code ) {
    ...
    var rem_amount = jQuery('#remaining_amount').val();
        ...
        var params = {'orderNo' : api_orderno ,'pluginInc': jQuery('#nn_plugin_include').val(), 'apiStatus' : refund_code , 'refundMethod' : refund_choice, 'accountHolderSepa' : sepa_holder , 'accountNumberSepa' : sepa_accno , 'bankCodeSepa' : sepa_bankcode , 'refundAmount' : amount_refund, 'refundRef' : refund_ref};
        ajax_call(params);
}
...
function subscription_cancel( api_orderno , subs_code ) {
    ...
    var params = {'orderNo' : api_orderno , 'pluginInc': jQuery('#nn_plugin_include').val(), 'apiStatus' : subs_code , 'subsReason' : termination_reason }
    ajax_call(params);
}
...
function ajax_call(params) {
    ...
    setTimeout(function(){
        if ('XDomainRequest' in window && window.XDomainRequest !== null) {
            ...
            xdr.open('GET', jQuery( '#nn_lib_url' ).val() + 'Novalnet.extensions.php?'+query);
            xdr.onload = function () {
            ...
            jQuery.ajax({
                url        : jQuery( '#nn_lib_url' ).val() + 'Novalnet.extensions.php',
                type       : 'post',
                dataType   : 'html',
...
function adminDisplayResponse(order_no) {
    var params = {'orderNo' : order_no ,'pluginInc': jQuery('#nn_plugin_include').val()};
    jQuery.ajax({
            url        : jQuery('#nn_plugin_url').val() + 'transactions.php',
            type       : 'post',
            data       :  params ,
            global     :  false,
            async      :  false,
...
function adminDisplayResponseCrossDomain(order_no) {
    var xdr = new XDomainRequest(); // Use Microsoft XDR
    xdr.open('GET', jQuery('#nn_plugin_url').val() + 'transactions.php?orderNo='+order_no);
    xdr.onload = function () {
