...
class novalnet_prepayment extends novalnetgateway {
  public $vendorid;
  public $productid;
  public $authcode;
  public $tariffid;
  public $testmode;
  public $payment_name = 'novalnet_prepayment';

  function init() {
    global $oPlugin,$hinweis;

    $this->typ = 'prepayment';
    $this->name = NOVALNET_PREPAYMENT_WAWI_NAME;
  ...
  function preparePaymentProcess($order) {
    ...
    if ($order_update == true) {
      $Zahlung_vor_Bestell = $this->checkOrderOnUpdate($order, 'novalnetvorkasse');
    }
    ...
    if (isset( $order_no ) && !empty( $order_no )) {
      ...
      $data['invoice_ref'] = 'BNR-' . $this->productid . '-' . $order->cBestellNr;
    }
    ...
    if (isset($parsed['status']) && $parsed['status'] != 100 && $order_update == true) {
      ...
      header( 'Location:' . URL_SHOP . '/novalnet_return.php?status_desc='. $_SESSION['novelnet']['error'] );
      exit;
    }
  ...
  function finalizeOrder($order, $hash, $args) {
    $response = $_SESSION['novalnet']['success'];
     $comments       = $this->updateOrderComments( $_SESSION['novalnet']['success'], $order, $this->payment_name );
    $_POST["kommentar"] = $_SESSION['kommentar'].$comments;
