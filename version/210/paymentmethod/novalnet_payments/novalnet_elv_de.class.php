...
#                                                       #
#  Script : novalnet_elv_de.class.php                   #
#                                                       #
#########################################################
...
class novalnet_elv_de extends novalnetgateway {
  ...
  public $productid_2;
  public $tariffid_2;
  public $payment_name = 'novalnet_elvde';

  function init() {
    global $oPlugin, $hinweis;

    $this->name = NOVALNET_ELVDE_WAWI_NAME;
  ...
  function handleAdditional($aPost_arr) {
    ...
    $smarty->assign('lang_elv_de_test_mode_info', '');
    if ( $GLOBALS['oSprache']->cISOSprache == 'ger' ){
		$lang_payment_name = ($_SESSION['Zahlungsart']->angezeigterName['ger'] != '') ? $_SESSION['Zahlungsart']->angezeigterName['ger'] : NOVALNET_ELVDE_ADDITIONAL_NAME;
	}else {
		$lang_payment_name = ($_SESSION['Zahlungsart']->angezeigterName['eng'] != '') ? $_SESSION['Zahlungsart']->angezeigterName['eng'] : NOVALNET_ELVDE_ADDITIONAL_NAME;
	}
	unset($_SESSION['novalnet']['nnat_holdername'],$_SESSION['novalnet']['nnat_accountnumber'], $_SESSION['novalnet']['nnat_bankcode'], $_SESSION['novalnet']['sepa_field_validator'], $_SESSION['novalnet']['nn_sepapanhash'], $_SESSION['novalnet']['sepa_mandate_ref'], $_SESSION['novalnet']['sepa_mandate_date'], $_SESSION['novalnet']['cc_pan_hash'], $_SESSION['novalnet']['cc_unique_id'], $_SESSION['novalnet']['cc_fldvalidator']);
	...
	if (isset($aPost_arr['nn_holdername']) && !empty($aPost_arr['nn_holdername']) ){
		...
		$_SESSION['novalnet']['nnde_holdername'] = (($this->autoFillFields) ? $aPost_arr['nn_holdername'] : ''); 
		$aPost_arr['nn_accountnumber'] = isset($aPost_arr['nn_accountnumber']) ? str_replace(' ', '', trim($aPost_arr['nn_accountnumber'])) : '';
		$_SESSION['novalnet']['nnde_accountnumber'] = (($this->autoFillFields) ? $aPost_arr['nn_accountnumber'] : '');
		$aPost_arr['nn_bankcode']	= isset($aPost_arr['nn_bankcode']) ? str_replace(' ', '', trim($aPost_arr['nn_bankcode'])) : '';
		$_SESSION['novalnet']['nnde_bankcode'] = (($this->autoFillFields) ? $aPost_arr['nn_bankcode'] : '');
		$aPost_arr['nn_acdc']		= isset($aPost_arr['nn_acdc']) ? str_replace(' ', '', trim($aPost_arr['nn_acdc'])) : '';
		$_SESSION['novalnet']['nnde_acdc'] = (($this->autoFillFields) ? $aPost_arr['nn_acdc'] : '');
	}
	$de_acc_name  = (isset($_SESSION['novalnet']['nnde_holdername']) ? $_SESSION['novalnet']['nnde_holdername'] : '');
	$de_acc_no 	  = (isset($_SESSION['novalnet']['nnde_accountnumber']) ? $_SESSION['novalnet']['nnde_accountnumber'] : '');
	$de_bank_code = (isset($_SESSION['novalnet']['nnde_bankcode']) ? $_SESSION['novalnet']['nnde_bankcode'] : '');
	$de_acdc = (isset($_SESSION['novalnet']['nnde_acdc']) ? $_SESSION['novalnet']['nnde_acdc'] : '');
	
    $smarty->assign( 'lang_payment_name', $lang_payment_name );
    $smarty->assign( 'de_acc_name', $de_acc_name);
    $smarty->assign( 'de_acc_no', $de_acc_no);
    $smarty->assign( 'de_bank_code', $de_bank_code);
    $smarty->assign( 'de_acdc', $de_acdc);
    ...
    $smarty->assign( 'lang_acdc', NOVALNET_ELVDE_ACCOUNT_ACDC );
    $smarty->assign( 'acdc', $this->acdc );
    $smarty->assign( 'lang_de_description', NOVALNET_ELVATDE_PAYMENT_DESCRIPTION );
    if ($this->testmode == '1') {
        $smarty->assign('lang_elv_de_test_mode_info', NOVALNET_TESTMODE_MSG);
      }
