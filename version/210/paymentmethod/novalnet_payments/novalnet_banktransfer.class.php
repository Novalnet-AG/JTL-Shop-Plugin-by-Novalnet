...
class novalnet_banktransfer extends novalnetgateway {
  ...
  function __construct() {
    ...
    $this->name = NOVALNET_INSTANT_WAWI_NAME;
    $this->doAssignConfigVarsToMembers();
    $this->setError();
    if ($_SESSION['novalnet']['nWaehrendBestellung'] == 'Nein') {  
      if ($_REQUEST['status'] && $_REQUEST['paymentname'] == 'novalnet_bank') {
        $this->orderComplete($_REQUEST);
      }
    }
    if ($_REQUEST['status'] != 100 && !empty( $_REQUEST['status_text'] ) && $_REQUEST['paymentname'] == 'novalnet_bank') { 
