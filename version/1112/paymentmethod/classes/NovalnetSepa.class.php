...
class NovalnetSepa extends PaymentMethod
{
    ...
    public function getSepaRefillHash()
    {
        ...
        if ($this->novalnetGateway->helper->getConfigurationParams('sepa_refill')) { // Condition to check if the SEPA payment refill is enabled
            ...
            $storedHash = $DB->$shopQuery('SELECT cSepaHash FROM xplugin_novalnetag_tnovalnet_status WHERE cMail = "' . $_SESSION['Kunde']->cMail . '" ORDER BY kSno DESC LIMIT 1', 1);
        }

        return (($this->novalnetGateway->helper->getConfigurationParams('sepa_autorefill') && !empty($_SESSION[$this->paymentName]['nn_payment_hash'])) ? $_SESSION[$this->paymentName]['nn_payment_hash'] : (!empty($storedHash) ? $storedHash->cSepaHash : ''));
    }
