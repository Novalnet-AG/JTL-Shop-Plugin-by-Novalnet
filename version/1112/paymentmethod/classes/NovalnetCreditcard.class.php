...
class NovalnetCreditcard extends PaymentMethod
{
    ...
    public function handleAdditional($aPost_arr)
    {
        ...
        $smarty->assign( array(
            ...
            'cc3dactive'            => $this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode') // Verify cc3d is active
        ) );

        if ($this->novalnetGateway->helper->getConfigurationParams('extensive_option', $this->paymentName) == '1' && !empty($referenceTid) && !$this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode')) { // Condition to display saved card details
    ...
    public function preparePaymentProcess($order)
    {
        ...
        if (empty($_SESSION[$this->paymentName]['one_click_shopping'])) { // Condition to check reference transaction
            ...
            if (($extensiveOption == '1' && !$this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode')) || $extensiveOption == '2' ) {
        ...
        if ($this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode')) { // If the credit card is 3D secured
    ...
    public function finalizeOrder($order, $hash, $args)
    {
        ...
        return $this->novalnetGateway->verifyNotification($order, $this->paymentName, $this->paymentKey, $this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode') ? $args : ''); // Finalises the order based on response
    }
    ...
    public function handleNotification($order, $paymentHash, $args)
    {
        ...
        if (is_object($incomingPayment) && intval($incomingPayment->kZahlungseingang) > 0) {
            ...
            $this->novalnetGateway->handlePaymentCompletion($order, $this->generateHash($order), $this->paymentKey, $this->paymentName, $this->novalnetGateway->helper->getConfigurationParams('cc3d_active_mode') ? $args : ''); // Completes the order
        }
    }
    ...
    {
        if ($argsArray['tid_status'] == 100 && $this->novalnetGateway->helper->getConfigurationParams('extensive_option', $this->paymentName) != '2') { // Adds to incoming payments only if the status is 100 or completed
    ...
    {
        return base64_encode($this->novalnetGateway->helper->getConfigurationParams('novalnet_public_key') . '&' . nnGetIpAddress('REMOTE_ADDR') . '&' . nnGetIpAddress('SERVER_ADDR'));
    }
