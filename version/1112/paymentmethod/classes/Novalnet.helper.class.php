...
class NovalnetHelper
{
    ...
    public function generateEncode($data)
    {
        ...
        try {
            ...
            $data = $crc . '|' . $data;
            $data = bin2hex($data . $_SESSION['nn_key_password']);
    ...
    {
        return md5($h['auth_code'] . $h['product'] . $h['tariff'] . $h['amount'] . $h['test_mode'] . $h['uniqid'] . strrev($_SESSION['nn_key_password']));
    }
    ...
    public function generateDecode($data)
    {
        try {
            ...
            $data = substr($data, 0, stripos($data, $_SESSION['nn_key_password']));
            $pos  = strpos($data, '|');
    ...
    {
        $sessionValues = array('booking', 'error', 'amount', 'payment', 'request', 'during_order', 'comments', 'key_password', $payment . '_tid', $payment . '_guarantee', $payment . '_guarantee_error');
...
function nnGetIpAddress($addressType)
{
    if ($addressType == 'REMOTE_ADDR') {
        ...
        if ($remoteAddress == '0.0.0.0') {
            return '127.0.0.1';
        }

        return (filter_var($remoteAddress, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) ? '127.0.0.1' : $remoteAddress);
    }

    return (filter_var($_SERVER[$addressType], FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) ? '127.0.0.1' : $_SERVER[$addressType]);
}
