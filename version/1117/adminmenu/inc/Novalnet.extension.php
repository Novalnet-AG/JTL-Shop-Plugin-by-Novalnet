        ...
        switch ($request['apiStatus']) {
            ...
            case 'void':
                if ($request['apiStatus'] == 'capture') {
                    ...
                    $comment  = $orderObj->cZahlungsartName . PHP_EOL;
                    if($configDb['key']== 41){
					$comment .= $novalnetOrderLanguage['__NN_guarantee_payments_comments'] . PHP_EOL;
					}
                    $comment .= $novalnetOrderLanguage['__NN_tid_label'] . $orderDetails->nNntid . PHP_EOL;

                    if ($transDetails->bTestmodus) {
                        $comment .= $novalnetOrderLanguage['__NN_test_order'] . PHP_EOL;
                    }
                    ...
                    $comment .= $novalnetGateway->formInvoicePrepaymentComments(
					  ...
					  NovalnetGateway::performDbExecution('tbestellung', 'cKommentar = CONCAT(cKommentar, "' . PHP_EOL .$comment . '")', 'cBestellNr = "' . $orderNo . '"'); // Updates the value into the database		
					  $sendOrderMail = true;
            ...
            case 'amountUpdate':
                if (!in_array($paymentMethod, array('novalnet_sepa', 'novalnet_cashpayment'))) {
                    ...
                    $comments  = $orderObj->cZahlungsartName . PHP_EOL;
            ...
            default:
                $transactionResponseMessage = PHP_EOL . sprintf(
