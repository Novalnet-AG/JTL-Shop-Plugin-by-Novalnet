...
{
    var bank_country   = jQuery('#nn_sepa_country').val();
    var account_holder = removeUnwantedSpecialChars(jQuery('#nn_sepaowner').val());
    var account_no     = jQuery('#nn_sepa_account_no').val();
    var bank_code      = jQuery('#nn_sepa_bank_code').val();
...
function sepaHashCall()
{
    ...
    var bank_country   = jQuery('#nn_sepa_country').val();
    var account_holder = removeUnwantedSpecialChars(jQuery('#nn_sepaowner').val());
    var iban           = jQuery('#nn_sepa_account_no').val().replace(/[^a-z0-9]+/gi, '');
    var bic            = jQuery('#nn_sepa_bank_code').val().replace(/[^a-z0-9]+/gi, '');
    var nn_sepa_iban   = jQuery('#nn_sepa_iban').val();
    var nn_sepa_bic    = jQuery('#nn_sepa_bic').val();
