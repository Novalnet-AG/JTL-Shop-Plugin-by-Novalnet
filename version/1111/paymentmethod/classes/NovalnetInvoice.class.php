...
class NovalnetInvoice extends PaymentMethod
{
    ...
    public function preparePaymentProcess($order)
    {
        ...
        if (!empty($_SESSION['nn_' . $this->paymentName . '_tid'])) { // Performs second call for fraud module after payment call success
            ...
            if ($_SESSION['Zahlungsart']->nWaehrendBestellung == 0) {
                ...
                $this->novalnetGateway->verifyNotification($order, $this->paymentName, !empty($_SESSION['nn_' . $this->paymentName . '_guarantee']) ? 41 : 27);
    ...
    public function finalizeOrder($order, $hash, $args)
    {
        ...
        // Finalises the order based on response
        return $this->novalnetGateway->verifyNotification($order, $this->paymentName, !empty($_SESSION['nn_' . $this->paymentName . '_guarantee']) ? 41 : 27);
    }
    ...
    public function handleNotification($order, $paymentHash, $args)
    {
        ...
        $this->novalnetGateway->handlePaymentCompletion($order, $this->generateHash($order), !empty($_SESSION['nn_' . $this->paymentName . '_guarantee']) ? 41 : 27, $this->paymentName); // Completes the order
    }
