...
class NovalnetVendorScript {
    ...
    public function addOrderObjectValues(&$order)
    {
		...
		$orderAmount = $DB->$shopQuery('SELECT nBetrag FROM xplugin_novalnetag_tnovalnet_status WHERE nNntid = "' . $this->arycaptureparams['shop_tid'] . '"', 1);

        $order->kPaymentId   = nnGetPaymentModuleId($order->kZahlungsart);
        $order->cPaymentType = $this->getPaymentMethod($order->kPaymentId);
        $order->fGesamtsumme = !empty($orderAmount->nBetrag) ? $orderAmount->nBetrag : gibPreisString($order->fGesamtsumme) * 100;
