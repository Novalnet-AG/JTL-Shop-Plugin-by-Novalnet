...
class NovalnetBanktransfer extends PaymentMethod
{
     ...
     * @var string
     */
    public $paymentName = 'novalnet_banktransfer';

    /**
     * @var string
     */
    public $paymentKey = 33;
    ...
    public function __construct($moduleID)
    {
        ...
        $this->moduleID = 'kPlugin_' . $this->novalnetGateway->helper->oPlugin->kPlugin . '_novalnetsofortüberweisung';
    ...
    public function isValidIntern($args_arr = array())
    {
        ...
        $this->novalnetGateway->setupShopInformationText($this->moduleID, 'banktransfer');
    ...
    public function handleAdditional($aPost_arr)
    {
        ...
        $smarty->assign(array(
            ...
            'nnLang'      => nnGetLanguageText(
                array(
                                    '__NN_testmode',
                                    '__NN_redirection_text',
                                    '__NN_redirection_browser_text'
                                )
            ) // Gets language texts for the variables
        ));
    ...
    public function preparePaymentProcess($order)
    {
        ...
        // Encodes the basic payment parameters before sending to third party
        $this->novalnetGateway->helper->generateEncodeArray($paymentRequestParameters);
        $paymentRequestParameters['hash'] = $this->novalnetGateway->helper->generateHashValue($paymentRequestParameters, $this->paymentKey);
        $smarty->assign(array(
            ...
            'shopLatest'        => $shopVersion == '4x',
            'paymentMethodPath' => $shopUrl . '/' . PFAD_PLUGIN . $this->novalnetGateway->helper->oPlugin->cVerzeichnis.
