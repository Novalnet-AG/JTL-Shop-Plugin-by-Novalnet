...
class NovalnetGateway
{
    ...
    public function generatePaymentParams($order, $payment)
    {
        ...
        if ($this->tariffValues[1] != 2) { // If the tariff ID configured is a subscription tariff id
            ...
            // Condition to check if the tariff period2 values can be passed to server
            if ($tariffPeriodAmount != '' && $tariffPeriod2 != '') {
    ...
    public function redirectOnError($order, $response, $paymentName)
    {
        ...
        if ($_SESSION['Zahlungsart']->nWaehrendBestellung == 0 || !empty($_SESSION['nn_during_order'])) {
            // Article restock
            foreach ($order->Positionen as $orderArticles) {
                $fLagerbestand = $orderArticles->nAnzahl + $orderArticles->Artikel->fLagerbestand;
                self::performDbExecution('tartikel', 'fLagerbestand = "'.$fLagerbestand.'"', 'cArtNr ="'.$orderArticles->Artikel->cArtNr.'"');
             }
    ...
    public function formTransactionComments($response, $order, $paymentName, $onError = false)
    {
        ...
        if (!$onError) {
            ...
            if ($paymentName == 'novalnet_invoice' && $response['tid_status'] == '100') {
            ...
            } elseif ($paymentName == 'novalnet_prepayment') {
                $invoicePaymentsComments = $this->formInvoicePrepaymentComments($response, $order->Waehrung->cISO);
                $transactionComments    .= $invoicePaymentsComments;
            ...
            if ($response['tid_status'] == '75' && $paymentName=='novalnet_invoice') {
                $transactionComments .= PHP_EOL . $this->helper->oPlugin->oPluginSprachvariableAssoc_arr['__NN_gurantee_pending_payment_text'];
            }
        }
    ...
    public function formInvoicePrepaymentComments($response, $currency, $updateAmount = false)
    {
        ...
        $referenceParams = $updateAmount
                            ? unserialize($response['referenceValues'])
                            : $this->helper->getInvoicePaymentsReferences($paymentName);
        ...
        $transComments .= (($refCount['1'] > 1)
                            ? $novalnetOrderLanguage['__NN_invoice_payments_multiple_reference_text']
                            : $novalnetOrderLanguage['__NN_invoice_payments_single_reference_text']) . PHP_EOL;

        foreach ($referenceParams as $key => $val) {
            if (!empty($val)) {
                $suffix = (nnLoadLanguageIso($response['kSprache']) == 'ger' && $refCount['1'] > 1)
                            ? $i . '. '
                            : ($refCount['1'] > 1 ? ' ' .$i : '');
                $transComments .= sprintf($novalnetOrderLanguage['__NN_invoice_payments_reference'], $suffix)
                                    . ': ' . $referenceSuffix[$key] . PHP_EOL;
                $i+=1;
            }
        }
    ...
    public function prepareCashpaymentComments($response)
    {
        ...
        $transComments  = ($response['cashpayment_due_date'] != '')
    ...
    public function setupAdditionalInfoData($response, $paymentKey, $referenceOrder, $paymentName)
    {
        if (!in_array($paymentKey, array('27', '41')) && ($paymentKey != '59'
            ...
            || $referenceOrder || empty($_SESSION['Kunde']->kKunde)))) {
            return '';
        }
