...
class NovalnetInvoice extends PaymentMethod
{
    ...
    public function updateShopDatabase($order)
    {
        ...
        if (!empty($_SESSION['nn_' . $this->paymentName . '_guarantee']) && $_SESSION[$this->paymentName]['tid_status'] == '100') {
    ...
    {
        $date='';
        if (nnIsDigits($this->novalnetGateway->helper->getConfigurationParams('invoice_duration'))) {
            $day = $this->novalnetGateway->helper->getConfigurationParams('invoice_duration');
