...
function sepaHashCall()
{
    ...
    bank_country = jQuery('#nn_sepa_country').val();
    account_holder = removeUnwantedSpecialChars(jQuery('#nn_sepaowner').val());
    iban = jQuery('#nn_sepa_account_no').val();
    bic = jQuery('#nn_sepa_bank_code').val();
...
function sepaRefillFormcall()
{
    ...
    var refillRequestParams = {'vendor_id' : jQuery('#novalnet_vendor').val() , 'vendor_authcode' : jQuery('#novalnet_authcode').val() , 'unique_id' : jQuery('#nn_sepaunique_id').val(), 'sepa_data_approved' : 1, 'mandate_data_req' : 1, 'sepa_hash' : refillpanhash};
