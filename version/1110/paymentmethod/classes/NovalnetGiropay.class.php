...
class NovalnetGiropay extends PaymentMethod
{
    ...
    {
        $this->paymentName = 'novalnet_giropay';
    ...
    public function preparePaymentProcess($order)
    {
        ...
        $paymentRequestParameters['key'] = 69;
        ...
        $paymentRequestParameters['hash'] = $this->helper->generateHashValue($paymentRequestParameters, 50); // Encodes the basic payment parameters before sending to third party
    ...
    {
        return $this->novalnetGateway->verifyNotification($order, $this->paymentName, 69, $args); // Finalises the order based on response
    }
    ...
    public function handleNotification($order, $paymentHash, $args)
    {
        ...
        $this->novalnetGateway->handlePaymentCompletion($order, $this->generateHash($order), 69, $this->paymentName, $args); // Completes the order
    }
