...
{
    global $processTestMode, $processDebugMode, $oPlugin, $jtlPaymentClass, $DB, $shopQuery, $novalnetOrderLanguage;
    ...
    else
    {
        ...
        switch ($paymentLevel)
        {
            case 2: // Level 2 payments - Types of collections available
                ...
                if ($paymentSuccess)
                {
                    if (in_array($jtlVendorParams['payment_type'], array('INVOICE_CREDIT', 'ONLINE_TRANSFER_CREDIT', 'GUARANTEED_INVOICE_CREDIT')))
                    {
                        ...
                        {
                            $callbackScriptText = sprintf($novalnetOrderLanguage['__NN_callback_initial_execution'], $jtlVendorParams['shop_tid'], gibPreisString($jtlVendorParams['amount'] / 100) . ' ' . $jtlVendorHistory->cCurrencyISO, date('d.m.Y'), date('H:i:s'), $jtlVendorParams['tid']);
                            ...
                            if ($orderPaidAmount >= $orderAmount)
                            {
                                ...
                                if ($jtlVendorParams['payment_type'] == 'ONLINE_TRANSFER_CREDIT') {
									...
									$callbackScriptText = sprintf($novalnetOrderLanguage['__NN_callback_status_change'], gibPreisString($jtlVendorParams['amount'] / 100) . ' ' . $jtlVendorHistory->cCurrencyISO, $orderId);
            ...
            case 1: // Level 1 payments - Types of chargebacks
                ...
                if ($paymentSuccess)
                {
                    $callbackScriptText = in_array($jtlVendorParams['payment_type'], array('CREDITCARD_BOOKBACK', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND')) ? sprintf($novalnetOrderLanguage['__NN_callback_bookback_execution'], $jtlVendorParams['shop_tid'], gibPreisString($jtlVendorParams['amount'] / 100) . ' ' . $jtlVendorHistory->cCurrencyISO, date('d.m.Y'), date('H:i:s'), $jtlVendorParams['tid']) : sprintf($novalnetOrderLanguage['__NN_callback_chargeback_execution'], $jtlVendorParams['shop_tid'], gibPreisString($jtlVendorParams['amount'] / 100) . ' ' . $jtlVendorHistory->cCurrencyISO, date('d.m.Y'), date('H:i:s'), $jtlVendorParams['tid']);
            ...
            case 0: // Level 0 payments - Types of payments
                ...
                if ($jtlVendorParams['status'] == 100 && in_array($jtlVendorParams['tid_status'], array(85, 90, 91, 98, 99, 100)))
                {
                    if (!empty($jtlVendorParams['subs_billing']))
                    {
                        $callbackScriptText = sprintf($novalnetOrderLanguage['__NN_callback_subscription_execution'], $jtlVendorParams['shop_tid'], gibPreisString($jtlVendorParams['amount'] / 100) . ' ' . $jtlVendorHistory->cCurrencyISO, date('d.m.Y'), date('H:i:s'), $jtlVendorParams['tid']);
                    ...
                    elseif (in_array($jtlVendorParams['payment_type'], array('PAYPAL', 'PRZELEWY24')))
                    {
                        ...
                        if ($query->kCallbackAmount < $orderAmount)
                        {
                            ...
                            NovalnetGateway::performDbExecution('tbestellung', 'dBezahltDatum = now(), cStatus=' . constant($oPlugin->oPluginEinstellungAssoc_arr[$jtlVendorHistory->cPaymentType.'_set_order_status']), 'cBestellNr = "' . $orderId . '"'); // Updates the value into the database
...
class NovalnetVendorScript {
     ...
     * @var array
     */
    protected $aryPayments = array('CREDITCARD','INVOICE_START', 'DIRECT_DEBIT_SEPA','GUARANTEED_DIRECT_DEBIT_SEPA', 'GUARANTEED_INVOICE_START','PAYPAL','ONLINE_TRANSFER','IDEAL','EPS', 'GIROPAY');

    /**
     * @Type of Chargebacks available - Level : 1
     * @var array
     */
    protected $aryChargebacks = array('RETURN_DEBIT_SEPA','REVERSAL', 'CREDITCARD_BOOKBACK','CREDITCARD_CHARGEBACK','REFUND_BY_BANK_TRANSFER_EU', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND');
    ...
    protected $aPaymentTypes = array(
        'novalnet_invoice'      => array('INVOICE_START', 'INVOICE_CREDIT', 'GUARANTEED_INVOICE_START', 'GUARANTEED_INVOICE_CREDIT', 'SUBSCRIPTION_STOP'),
     ...
     * @return none
     */
    public function displayMessage($errorMsg, $forceDisplay = false)
    {
    ...
    public function validateCaptureParams($callbackRequestParams)
    {
        ...
        if (!in_array(getRealIp(), $this->ipAllowed) && !$processTestMode) { // Condition to check whether the callback is called from authorized IP
            $this->displayMessage('Unauthorised access from the IP ' . getRealIp(), true);
        }
        ...
        if (!empty($callbackRequestParams['vendor_activation'])) {
            ...
            if ($callbackRequestParams['payment_type'] != 'SUBSCRIPTION_STOP' && (!is_numeric( $callbackRequestParams['amount']) || $callbackRequestParams['amount'] < 0)) {
                $this->displayMessage('Novalnet callback received. The requested amount ('. $callbackRequestParams['amount'] .') is not valid');
            }
    ...
    public function handleCommunicationBreak($order, $callbackRequestParams)
    {
        ...
        if ($callbackRequestParams['status'] == 100 && in_array($callbackRequestParams['tid_status'], array(85, 86, 90, 91, 98, 99, 100))) { // Condition to check communication failure for the payment success
            ...
            if (in_array($callbackRequestParams['tid_status'], array(86, 90))) {
                ...
                NovalnetGateway::performDbExecution('tbestellung', 'dBezahltDatum = now(), cKommentar = CONCAT(cKommentar, "' . $transactionComments . '"), cStatus=' . constant($oPlugin->oPluginEinstellungAssoc_arr[$order->cPaymentType.'_set_order_status']), 'cBestellNr = "' . $orderId . '"'); // Updates the value into the database
            }
        ...
        $jtlPaymentClass->insertOrderIntoDBForFailure(array('tid' => $callbackRequestParams['shop_tid'], 'payment' => $order->cPaymentType, 'email' => $callbackRequestParams['email'] ,'status' => $callbackRequestParams['tid_status']), $order); // Insert the order details into Novalnet table
    ...
    public function callbackFinalProcess($callbackScriptText, $orderReference, $greaterAmount = false)
    {
        ...
        NovalnetGateway::performDbExecution('tbestellung', 'cKommentar = CONCAT(cKommentar, "' . PHP_EOL . $callbackScriptText .PHP_EOL . '")', 'cBestellNr = "' . $orderReference->cBestellNr . '"'); // Updates the value into the database
    ...
    public function logCallbackProcess($orderReference)
    {
        ...
        $insertCallback->nCallbackAmount = ($this->arycaptureparams['tid_status'] == 90) ? 0 : ((in_array($orderReference->cPaymentType, array('novalnet_paypal', 'novalnet_przelewy24'))) ? ($orderAmount->fGesamtsumme * 100) : $this->arycaptureparams['amount']);
    ...
    {
        $paymentMethods = array('novalnetkaufaufrechnung' => 'novalnet_invoice', 'novalnetvorauskasse' => 'novalnet_prepayment', 'novalnetpaypal' => 'novalnet_paypal','novalnetkreditkarte' => 'novalnet_cc', utf8_decode('novalnetsofortüberweisung') => 'novalnet_banktransfer', 'novalnetideal' => 'novalnet_ideal', 'novalneteps' => 'novalnet_eps', 'novalnetlastschriftsepa' => 'novalnet_sepa', 'novalnetgiropay' => 'novalnet_giropay', 'novalnetprzelewy24' => 'novalnet_przelewy24');
    ...
    public function addOrderObjectValues(&$order)
    {
        ...
        $order->fGesamtsumme = gibPreisString($order->fGesamtsumme) * 100;
