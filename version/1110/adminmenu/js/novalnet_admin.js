...
function fillMerchantConfiguration()
{
    ...
    var autoconfigurationRequestParams = {'system_ip' : jQuery('#system_ip').val(), 'api_config_hash' : jQuery('input[name=novalnet_public_key]').val(), 'lang' : 'DE'};
...
function transactionResponseHandler(type, result, transactionResponseParams)
{
    ...
    switch(type) {
        ...
        case 'api':
            ...
            transactionRequestHandler(extensionResponseParams, jQuery('#nn_plugin_url').val() +'transactions.php?', jQuery('#nn_plugin_url').val() + 'transactions.php', 'orders');
        break;
