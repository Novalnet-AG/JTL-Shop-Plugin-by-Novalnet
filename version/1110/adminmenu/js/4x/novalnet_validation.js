...
jQuery(document).ready(function() {
	...
	jQuery.each(['#novalnet_invoice_guarantee', '#novalnet_sepa_guarantee'], function(index, element) {
		jQuery(element).closest('.panel-body').prepend(decodeTextMessage('<div class="input-group"><span class="input-group-addon nn_additional_span"><b><h5>Grundanforderungen für die Zahlungsgarantie</h5><br>Zugelassene Staaten: AT, DE, CH<br> Zugelassene Währung: EUR<br>Mindestbetrag der Bestellung >= 20,00 EUR<br>Maximalbetrag der Bestellung <= 5.000,00 EUR<br>Mindestalter des Endkunden >= 18 Jahre<br>Rechnungsadresse und Lieferadresse müssen übereinstimmen<br>Geschenkgutscheine / Coupons sind nicht erlaubt</b></span></div>'));
	});
    ...
    jQuery('#sepa_due_date, #manual_check_limit, #novalnet_invoice_guarantee_min_amount, #novalnet_invoice_guarantee_max_amount, #novalnet_sepa_guarantee_min_amount, #novalnet_sepa_guarantee_max_amount, #tariff_period, #tariff_period2, #tariff_period2_amount, #novalnet_public_key').parent().on('change', function() {
...
function performAdminValidations(event) {
	if (jQuery('#content').find('.tab-settings-0').hasClass('active')) {
		...
		} else if (jQuery.trim(jQuery('#tariff_period').val()) != '' && !((/^\d+(d|m|y){1}$/).test(jQuery('#tariff_period').val()))) {
		...
		} else if (jQuery.trim(jQuery('#manual_check_limit').val()) != '' && isNaN(jQuery('#manual_check_limit').val())) {
		...
		if (jQuery.trim(jQuery('#sepa_due_date').val()) != '' && (isNaN(jQuery('#sepa_due_date').val()) || jQuery('#sepa_due_date').val() < 7)) {
			...
			jQuery.each(['invoice', 'sepa'],function(index, value) {
				if (jQuery('#novalnet_'+value+'_guarantee').val() == 1) {
					...
					var maximum_guarantee_amount = jQuery.trim(jQuery('#novalnet_'+value+'_guarantee_max_amount').val());
					...
					if (minimum_guarantee_amount != '' && (minimum_guarantee_amount < 2000 || minimum_guarantee_amount > 500000)) {
						...
						handleErrorElement(jQuery('#novalnet_'+value+'_guarantee_max_amount').parent());
					}
				}
			});
		}
