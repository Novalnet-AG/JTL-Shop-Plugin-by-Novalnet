...
class NovalnetPrepayment extends PaymentMethod
{
    ...
    public function isValidIntern($args_arr = array())
    {
        ...
        $this->novalnetGateway->setupShopInformationText($this->moduleID, 'prepayment');
    ...
    public function handleNotification($order, $paymentHash, $args)
    {
        ...
        $this->insertOrderIntoInvPaymentsTable($order->cBestellNr);
    ...
    public function updateShopDatabase($order)
    {
        ...
        $this->updateNotificationID($order->kBestellung, $_SESSION[$this->paymentName]['tid']);
    ...
    public function insertOrderIntoInvPaymentsTable($orderno)
    {
        ...
        $insertPrepaymentDetails->cReferenceValues  = serialize(array('Payment reference1' => 'BNR-'. $_SESSION['nn_request']['product'] .'-'.  $orderno, 'Payment reference2' => $_SESSION[$this->paymentName]['tid']));
